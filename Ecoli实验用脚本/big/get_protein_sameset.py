...
def test(group, dic):
    """处理蛋白质group，判断该蛋白质group的Sameset是否有所需蛋白质, 且主蛋白是错的"""
    ...
    for i in range(1, len(group)):
        ...
        if segs[1] == 'SameSet': #考察该SameSet
            if segs[2][0 : 2] == 'gi' or segs[2][0 : 3] == 'CON' or test_coli(segs[2], dic):
                return True
        else:
            break
    return False
...
def get_protein_filter(Filename, dic):
    ...
    for i in range(0, len(f)):
        ...
        if segs[0].isdigit():  #group头
            if len(group_now) != 0 and  test(group_now, dic):  #判断是否该主蛋白是错的但是SameSet是对的
                num += 1
